...
pd.Plotter() \
    ...
    .set_ylabel("$t$ [μs]") \
    .set_title(title) \
    .savefig(outdir.joinpath(f"tweezer_amod_wf_da={ampl:.3f}.png")) \
    .close()

(
    pd.Plotter.new(figsize=[2.8, 1.5])
    .plot(t[::100], P012[::100], color="k", label="$P_0 + P_1 + P_2$")
    .plot(t[::100], P0[::100], label="$P_0$")
    .plot(t[::100], P1[::100], label="$P_1$")
    .plot(t[::100], P2[::100], label="$P_2$")
    .ggrid()
    .legend(fontsize="x-small", loc="lower left")
    .set_xlabel("Time [μs]")
    .set_ylabel("Probability")
    # .set_title(title)
    .savefig(outdir.joinpath(f"tweezer_amod_probs_da={ampl:.3f}.png"))
    .savefig(outdir.joinpath(f"tweezer_amod_probs_da={ampl:.3f}.pdf"))
    .show()
    .close()
)
